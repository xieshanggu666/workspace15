...
- **重复操作不重复扣费**：命令携带客户端生成的 `op_id`，`(match_id, op_id)`
  在 commands 与 idem_results 上各有唯一约束。所有结算都走单一入口
  `_apply_under_lock`（对局锁串行）：同 op_id 重发先命中内存幂等表直接返回；
  即使跨连接/跨进程在 SELECT 与 INSERT 之间竞争，提交时的 PRIMARY KEY 冲突会
  让命令、事件、检查点在**同一 SQLite 事务**整体回滚，再返回首次结果，绝不二次
  扣费/追加事件。
- **结算异常安全**：每条命令先在状态克隆上 fold 并提交数据库，事务成功后才
  替换权威内存状态；中途任何异常都不会污染进行中的对局。
- **定时器 epoch 防陈旧触发**：每次重排定时器自增代数；已越过 sleep、正在
  排队等锁的旧超时任务在锁内用 `epoch + 当前任务身份 + 截止时刻` 三重校验
  自我作废，玩家命令与超时竞争时绝不会重复结算。
- **一致的广播顺序**：待发事件与权威快照在对局锁内基于同一状态构造并推进
  客户端事件序号，网络发送移到锁外投递不可变载荷，不会读到半个状态。
